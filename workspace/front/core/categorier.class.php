...
class Categorier{
	...
	function updateCat($catData){
		...
		$listPropIdsArr = [];
		if( ( property_exists($catData,'props') ) && (is_array($catData->props)) ){
			foreach ($catData->props as $id=>$prop){
		...
		if( ( property_exists($catData,'deleted') ) && (is_array($catData->deleted)) ) {
	...
	public static function hasChildesFront($catId){
		$sql = "
    	  SELECT `id`
    	  FROM `categories`
    	  WHERE `parent` = ?
    	";
		$categories =  frontDb::prepare($sql, 'i', [$catId])->fetch_all(MYSQLI_ASSOC);
		return (!empty($categories)) ? true : false;
	}
	public static function getCatChildesFront($parentId){
		$sql = "
	      SELECT `id`,`title`,`c_desc`,`image`
	      FROM `categories`
	      WHERE `parent` = ?
    	";
		$outCats = [];
		$categories =  frontDb::prepare($sql, 'i', [$parentId])->fetch_all(MYSQLI_ASSOC);
		foreach ($categories as $cat){
			$outCat = $cat;
			$outCat['hasChild'] = self::hasChildesFront($cat['id']);
			$outCats[] = $outCat;
		}
		return (!empty($outCats)) ? $outCats : [];
	}
	public static function getCategoriesToFront($catId=null){
    $whereStr = "";
    $single = false;
    if(!empty($catId) &&  !is_null($catId)){
    	$whereStr = "WHERE `id` = {$catId}";
    	$single = true;
    }
		$sql = "SELECT `id`,`title`,`c_desc`,`image`,`parent` FROM `categories` {$whereStr}";

		if(!$single){
			$categories = frontDb::prepare($sql)->fetch_all(MYSQLI_ASSOC);
			return (!empty($categories)) ? $categories : [];
		}
		$category = frontDb::prepare($sql)->fetch_assoc();
		if(empty($category)) return [];
		$category['childes'] = self::getCatChildesFront($category['id']);
		$category['props'] = self::getCategoriesPropsToFront($category['id']);

		return $category;
	}
	public static function getCategoriesWithPropsToFront($catId){

	}
	public static function getCategoriesPropsToFront($catId){
		$sql = "
    	  SELECT `id`,`name`,`p_desc`,`p_type`,`sort`
    	  FROM `category_properties`
    	  WHERE `c_id` = ?
				ORDER BY `sort`
    	";
		$props = frontDb::prepare($sql,'i',[$catId])->fetch_all(MYSQLI_ASSOC);
		return (!empty($props)) ? $props : [];
	}
