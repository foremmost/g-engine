...
class Customers {
  ...
  function getTempUserByMail($userData){
	...
	function addMeta($uId,$userData){
		...
		$photo = $this->db->filter($userData['photo']);
		$sql = "
			INSERT INTO users_meta (`u_id`,`name`,`second_name`,`description`,`email`,`photo`)
			VALUES(?,?,?,?,?,?)
		";
		$this->db->prepare($sql,'isssss',[
			$uId,
			$name,
			$second_name,
			$description,
	...
	function getAll($userData){
		...
		$sql = "
			SELECT DISTINCT `users`.id,`login`,`group_id`,`users_meta`.name, `second_name`,`description`,`iin`,`phone`,`groups`.`value` as `group` ,`photo`
			FROM (users
