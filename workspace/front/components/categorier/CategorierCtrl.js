...
export class CategorierCtrl  extends Ctrl  {
    ...
    editCat(clickData){
    	const _ =  this;
    	_.model.categoryData['id'] = clickData['item'].dataset.catId;
    };
    setParent(parentId){
    	this.model.categoryData['parent'] = parentId;
    };
    ...
    addSaveBtn(value,name){
			...
			if(!value || !(value == _.model.dataCast[name])){
				if(!_.view.categorierSaveBtnTpl){
					let
						form = systemConts['content'].querySelector('.categorier-form'),
