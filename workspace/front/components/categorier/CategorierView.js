...
export class CategorierView extends View {
    ...
    /* Templates */
		...
		addPropListBtnTpl(){
			...
			return	_.el('BUTTON',{
				...
				childes:[
					_.el('IMG',{
						src:'/workspace/img/buttons/addPropList.svg'
					})
				]
			});
		}
    ...
    async setParentField(elem,parentId){
        ...
        if( ( parent['parent'] != 0)  ){
            ...
            elem.value = parent[`title`];
            _.clearCont(parentList);
            MainEventBus.trigger(_.componentName,'setParent',parent['id']);
            parentList.append(_.createEl('OPTION',null,{
                text: parent['id'],
    ...
    async editCat(clickData){
			...
			//console.log(_.сategorierAddBtnTpl);
			if(!_.сategorierAddBtnTpl){
    ...
    async backToTable(){
			...
			let
				...
				addBtn = _.getTpl('addPropListBtnTpl');
			_.clearCont(pageActions);
			pageActions.prepend(addBtn);
			TweenMax.fromTo(addBtn,.5,{opacity:0,x:'-50%'},{opacity:1,x: '0%'});
			await _.contentTpl();
			MainEventBus.trigger('languager','loadTranslate',{
				cont: systemConts['content']
			});
    ...
    async fillCategoriesListOutSide(wordData = {}){
			...
			return new Promise(async function (resolve) {
				let
					listCont =  _.el('UL',{class:'categorier-list page-list'}),
					catParents = wordData['items'] ? wordData['items'] : await _.model.getParentCategories({
						page: 1,
					}),
					rowsFragment = document.createDocumentFragment();
				for(let parent of catParents){
